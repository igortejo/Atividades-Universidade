...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private int max_index(int index, int left, int right) {
		if (this.comparator.compare(heap[index], heap[left]) > 0) {
			if (isValidIndex(right)) {
				if (this.comparator.compare(heap[index], heap[right]) < 0)
					return right;
			}
			return index;
	...
	private void heapify(int position) {
		...
		int iLeft = this.left(position), iRight = this.right(position), largest = position;

		if (this.isValidIndex(iLeft) && this.compare(largest, iLeft) < 0)
			largest = iLeft;
	...
	public void buildHeap(T[] array) {
		...
		for (int i = (int) heap[parent(index)]; i >= 0; i--) {
			heapify(i);
		}
