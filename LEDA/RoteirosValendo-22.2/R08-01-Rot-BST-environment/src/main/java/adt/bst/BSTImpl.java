...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private int heightRecursive(BSTNode<T> currentNode) {
		if (currentNode.isEmpty()) {
			...
			return 1 + Math.max(heightRecursive((BSTNode<T>) currentNode.getLeft()), heightRecursive((BSTNode<T>) currentNode.getRight()));
		}
	...
	private BSTNode<T> searchRecursive(BSTNode<T> currentNode, T element) {
			...
			if ( currentNode.isEmpty() || element.equals(currentNode.getData())) {
				return currentNode;
			}else if (element.compareTo(currentNode.getData()) < 0) {
				return searchRecursive((BSTNode<T>) currentNode.getLeft(), element);
			}else {
				return searchRecursive((BSTNode<T>) currentNode.getRight(), element);
	...
	private void insertRecursive (BSTNode<T> currentNode, T element) {
		...
		if (currentNode.isEmpty()) {
			...
			if (element.compareTo(currentNode.getData()) < 0) {
				insertRecursive((BSTNode<T>) currentNode.getLeft(), element);
			}else {
				insertRecursive((BSTNode<T>) currentNode.getRight(), element);
			}
		}
	}
	...
	private BSTNode<T> maximumRecursive (BSTNode<T> currentNode) {
		...
		if (!currentNode.getRight().isEmpty()) {
			return maximumRecursive((BSTNode<T>) currentNode.getRight());
		}
	...
	private BSTNode<T> minimumRecursive(BSTNode<T> currentNode) {
		...
		if (!currentNode.getLeft().isEmpty()) {
			return minimumRecursive((BSTNode<T>) currentNode.getLeft());
		}
		return currentNode;
	...
	public BSTNode<T> sucessor(T element) {
		...
		if (element != null) {
			return sucessorRecursive(root, element);
		}
		return null;
	}
		...
		if (element != null) {
			...
			if (!node.isEmpty()) {
				if (node.isLeaf()) { // Caso 1: nó a ser removido é folha
					...
					node.setLeft(null);
					node.setRight(null);
				} else if (node.getRight().isEmpty() || node.getLeft().isEmpty()) { // Caso 2: nó a ser removido tem um
																					// filho
